...
## 助教原始 System Requirement

> 本文件以下所有 Story 都以此截圖為主要對齊基準（MVP scope）。
> [CLAUDE.md](./CLAUDE.md) 中的 FR 為延伸版本，提供更完整的測試保護網但非交付下限。

![助教 System Requirement 原始截圖](./docs/assets/system-requirement.png)
...
- [x] **Given** 已取得 token **When** 後續操作受保護 endpoint **Expect** `Authorization: Bearer ...` 可通過；過期 → 401
      · 測試：散布於 [backend/**tests**/](./backend/src/__tests__/) 每組 endpoint 測 + [jwt.test.ts](./frontend/src/__test__/jwt.test.ts) + [authStore.test.ts](./frontend/src/__test__/authStore.test.ts)
